...
class Authentication:
    ...
    def login(self, application):
        ...
        if not self._session_expire:
            ...
            try:
                # The session request with the Synology NAS starts here
                session_request = requests.get(self._base_url + login_api, param, verify=self._verify)

                if self._debug:
                    print('Debug: Received Data; ',session_request)
                    print('Request Sent: ', self._base_url + login_api)

                self._sid = session_request.json()['data']['sid']
                self._session_expire = False
                if self._debug:
                    return 'User logged in. New session started.'
