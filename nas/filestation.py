...
class FileStation:
    ...
    def __init__(self, ip_address, port, username, password, secure=False, cert_verify=False, dsm_version=7, debug=True, otp_code=None):
        ...
        self.calculate_filesize_keyword = {}
    ...
    def search_start(self, folder_path=None, recursive=None, pattern=None, extension=None, filetype=None,
        ...
        self._search_taskid_list.append(self._search_taskid)

        return str(self._search_taskid)
    ...
    def start_dir_size_calc(self, path=None):
        ...
        self.taskids['size'] = f'"{response_id}"'
    ...
    def get_dir_status(self):
        ...
        try:
            taskid = self.taskids['size']
        except KeyError:
            raise NoTask('The TaskID for get_dir_status() does not exist. Did you call start_dir_size_calc(path) before calling this function? ')
    ...
    def start_delete_task(self, path=None, accurate_progress=None, recursive=None, search_taskid=None):
        ...
        return str(self._delete_taskid)
    ...
    def get_file(self, path=None) -> Generator:
        ...
        url = ('%s%s' % (self.base_url, api_path)) + '?api=%s&version=%s&method=download&path=%s&mode=%s&_sid=%s' % (
                api_name, info['maxVersion'], parse.quote_plus(path), 'download', self._sid)
        
        # Grabs the info we need & checks for errors
        r: requests.models.Response = requests.session().get(url, stream=True,verify=False)
        r.raise_for_status()
        ...
        dir_: list = os.listdir(fullpath)
        
        # Checks if a file with the same name already exists
        if filename in dir_:
                
                if filename.count('.') == 0:
                    for x in range(len(dir_)):
                        if filename[:file_ext]+f'_{x}' in dir_:
                            continue
                        else:
                            filename = filename[:file_ext]+f'_{x}'; break
                            
                else:
                    file_ext = filename.rfind('.')
                    
                    for x in range(len(dir_)):
                        if filename[:file_ext]+f'_{x}'+filename[file_ext:] in dir_:
                            continue
                        else:
